...
async def send_reset_password_message(email: str) -> None:
    token = create_access_token(email, minutes=settings.EMAIL_RESET_TOKEN_EXPIRE_MINUTES)
    message = generate_reset_password_message(email, token.access_token, settings.EMAIL_RESET_TOKEN_EXPIRE_MINUTES)
...
def generate_reset_password_message(email: str, token: str, expires_at: int) -> MessageSchema:
    subject = f"Reset password for {settings.APP_NAME} account"
    recipients = [email]

    verification_url = get_reset_password_url(token)
    template_body = {"email": email, "url": verification_url, "expires_at": expires_at}
...
def get_reset_password_url(token: str) -> str:
    return f"{settings.SERVER_HOST}/api/v1/auth/reset-password?token={token}"
